                ...
                {isMounted && items.length === 0 ? (
                    <div className="flex h-full flex-col items-center justify-center space-y-1">
                        <div aria-hidden="true" className="relative mb-4 h-40 w-40 text-muted-foreground">
                            <Image src='/images/empty_cart.jpg' fill loading="eager" alt="empty shopping cart"/>
                        </div>
                        <h2 className="font-bold text-2xl">Your Cart is empty</h2>
                        <p className="text-muted-foreground text-center pt-1">Nothing to show here yet!</p>
                    </div>
                ): null}
                <ul className={cn({
                                ...
                                <div className="flex-shrink-0">
                                    <div className="relative h-24 w-24">
                                        {typeof image !== "string" && image.url ? (
                                            <Image fill src={image.url} alt="Product Image" className="h-full w-full rounded-md object-cover object-center"/>
                                        ): null}
                                    </div>
                                </div>
                    ...
                    <div className="mt-6 space-y-4">
                        <div className="flex items-center justify-between">
                            ...
                            <p className="text-sm font-medium text-gray-900">
                                {isMounted   ? formatPrice(cartTotal) : <Loader2 className="h-4 w-4 animate-spin text-muted-foreground"></Loader2>}
                            </p>
                        </div>
                        ...
                        <div className="flex items-center justify-between border-t border-gray-200 pt-4">
                            ...
                            <div className="text-sm font-medium text-gray-900">
                                {isMounted ? formatPrice(fee) : <Loader2 className="h-4 w-4 animate-spin text-muted-foreground"></Loader2>}
                            </div>
                        </div>
                        ...
                        <div className="flex items-center justify-between border-t border-gray-200">
                            ...
                            {isMounted ? formatPrice(cartTotal + fee) : <Loader2 className="h-4 w-4 animate-spin text-muted-foreground"></Loader2>}
                            </div>
                        </div>
                    </div>
